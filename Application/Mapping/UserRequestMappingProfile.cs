    ...
    public UserRequestMappingProfile()
    {
        CreateMap<UserRequest, UserRequestDto>()
            .ForMember(dest => dest.Status,
                opt => opt.MapFrom(src => src.Status.ToString())); // Пример преобразования статуса
        ...
        CreateMap<GetUserRequestDto, UserRequest>()
            .ForMember(dest => dest.CreatedAt,
                opt => opt.MapFrom(src => src.CreatedDate)) // Явное указание маппинга для даты
            .ReverseMap()
            .ForMember(dest => dest.CreatedDate,
                opt => opt.MapFrom(src => src.CreatedAt)); // И наоборот для другого направления
